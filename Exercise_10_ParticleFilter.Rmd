...
data. The approach for the litterfall rate could also be improvedwith
...
To do this we'll set up some storage and loop over time where we calling
...
-   Each row is a different variable at a different time. The key
    variables are LAI estimate, QC code, and standard devivation on the
    LAI estimate.
...
Before we move on to the resampling particle filter, we need to define
an number of ancillary functions: `update.params`, that will help us
